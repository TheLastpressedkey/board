...
const SIDEBAR_WIDTH = 80; // Width of the left sidebar + margin
...
export function useBoards() {
  ...
  const addLinkCard = useCallback(async (position: { x: number; y: number } = { x: 0, y: 0 }, url: string) => {
    if (!currentBoard) return;
    ...
    setBoards(prevBoards => prevBoards.map(board => {
      ...
      const arrangedCards = cards.map((card, index) => {
        ...
        return {
          ...card,
          position: {
            x: SIDEBAR_WIDTH + CARD_MARGIN + col * (FIXED_CARD_WIDTH + CARD_MARGIN),
